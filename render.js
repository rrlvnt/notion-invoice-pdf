  ...
  switch (type) {
    ...
    case 'column_list': {
      const columns = (block.children || []).map(col => {
        ...
        // Use Notion's actual column width ratio instead of
        // forcing every column to have equal width.
        const widthRatio =
          typeof col.column?.width_ratio === 'number'
            ? col.column.width_ratio
            : 1;

        return `
          <div
            class="column"
            style="flex:${widthRatio} 1 0%;"
          >
            ${inner}
          </div>
        `;
...
function buildFilename(properties) {
  ...
  // BJ__08.31.26_INVOICE_WSC_Dry_Museum__NYC_2026
  const parts = [];
...
function buildHeaderHtml(properties) {
  ...
  const issued =
    formatDate(properties['Issued']?.date?.start);

  const due =
    formatDate(properties['Due']?.formula?.date?.start);

  const amount =
    formatCurrency(properties['Amount']?.number);
...
async function main() {
  ...
  const headerHtml =
    buildHeaderHtml(notionPage.properties);

  const filenameBase =
    buildFilename(notionPage.properties);
